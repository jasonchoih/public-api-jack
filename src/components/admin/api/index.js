...
export default () =>
{
    ...
    const columns = [
        ...
        [ 'Logo', v => <AdvancedImage sizes="100vw" className='h-auto w-8 m-auto' cldImg={cld.image(v[11])} />
            // <CldImage width="0" height="0" src={`https://res.cloudinary.com/ddv2aeipa/image/upload/v1719214653/${v[11]}`} sizes="100vw" alt={v[11]} className='h-auto w-8 m-auto' />  
        ] ,
                        ...
                        <FormControl key={k} className={v[2]} size="small">
                            ...
                            <Select 
                                ...
                                onChange={event=>{
                                    ...
                                    setValue(v[0], event.target.value, {shouldValidate: true})
                                    if(v[0]=='Category'){
                                        // console.log(_category.find(cat=>cat[0]==event.target.value))
                                        dispatch.models.SET({
